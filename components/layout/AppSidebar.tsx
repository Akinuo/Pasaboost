...
import {
  ...
  LogOut, Menu, X, Sun, Moon, Monitor, ChevronDown,
} from 'lucide-react'
...
export default function AppSidebar({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      <motion.aside
        className="fixed lg:relative inset-y-0 left-0 z-50 lg:z-auto flex flex-col w-64 bg-card border-r border-border lg:translate-x-0"
        initial={{ x: -256 }}
      ...
      >
        <div className="flex items-center justify-between px-6 py-5 border-b border-border">
          ...
          <button onClick={() => setSidebarOpen(false)} className="lg:hidden p-1.5 rounded-md hover:bg-accent">
            <X size={18} />
          </button>
        </div>
